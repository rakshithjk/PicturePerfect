...
func DisplayReview(w http.ResponseWriter, r *http.Request){
	...
	id := strings.TrimPrefix(r.URL.Path, "/tvshow/reviews/")
	...
	total_page,err := db.Query("select count(*) from tv_reviews")				//  Total_page contains result of query to find total number of records in the Database
	if err!=nil{
		 	fmt.Println(err)
		 }
	...
	var query = "select * from tv_reviews where show_id = "+id+" "+ordquery+" limit 10 offset "+strconv.Itoa((10*(page-1)))
	fmt.Println(query)
	rows,err:=db.Query(query)
	 ...
	 var tag review
	 var query1 = "select avg(rating) from tv_reviews where show_id = "+id
		...
		for rows1.Next(){
			err:=rows1.Scan(&list.Avg_Rating)
