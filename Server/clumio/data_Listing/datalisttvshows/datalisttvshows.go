...
func DataList(w http.ResponseWriter, r *http.Request) {
    ...
    total_page,err := db.Query("select count(*) from tvshows")               //  Total_page contains result of query to find total number of records in the Database
    if err!=nil{
            fmt.Println(err)
         }
    ...
    var query = "select * from tvshows "+ordquery+" limit 15 offset "+strconv.Itoa((15*(page-1)))    // Query string
    fmt.Println(query)
